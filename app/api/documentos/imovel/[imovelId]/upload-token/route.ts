...
const TIPOS_VALIDOS = ['contrato', 'escritura', 'planta', 'iptu', 'foto', 'vistoria', 'outro']
const MAX_DOCS_IMOVEL = 30
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if ((count ?? 0) >= MAX_DOCS_IMOVEL) {
    return NextResponse.json(
      { error: `Limite de ${MAX_DOCS_IMOVEL} documentos por imóvel atingido.` },
      { status: 422 },
    )
  }
  ...
  const storagePath = `usuarios/${user.id}/imoveis/${imovelId}/${Date.now()}_${nomeSeguro}`
