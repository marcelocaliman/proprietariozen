...
export async function POST(
  ...
  try {
    ...
    const body = await req.json() as { motivo?: string; tipoMotivo?: string }
    const { motivo, tipoMotivo } = body
      ...
      try {
        ...
        if (profile?.asaas_api_key_enc) {
          ...
          if (!res.ok && res.status !== 404) {
            asaasAviso = 'Cobrança cancelada localmente. Verifique manualmente no Asaas.'
          }
        }
      } catch {
        asaasAviso = 'Cobrança cancelada localmente. Verifique manualmente no Asaas.'
    ...
    const motivoCompleto = motivo?.trim()
