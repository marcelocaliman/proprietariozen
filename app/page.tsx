...
function AppMockup() {
  return (
    <svg width="300" height="560" viewBox="0 0 300 560" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-2xl" role="img" aria-label="Interface do ProprietárioZen">
...
const FEATURES = [
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>,
  ...
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
    title: 'Contratos e reajuste',
    desc: 'Cadastre contratos com vigência e reajuste automático por IGPM ou IPCA — sem precisar lembrar.',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>,
    title: 'Dashboard financeiro',
    desc: 'Visualize recebidos, pendentes e inadimplentes num painel limpo e organizado por mês.',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>,
    title: 'Alertas automáticos',
    desc: 'E-mails automáticos antes do vencimento, no atraso e na data de reajuste. Sem você precisar fazer nada.',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
    title: 'Gestão de inquilinos',
    desc: 'Histórico completo: dados de contato, período de locação e registro de todos os pagamentos.',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>,
    title: 'Recibos PDF',
  ...
  return (
    <>
      <style>{`
        ...
        .lp-step-connector::after { content:''; position:absolute; top:40px; left:calc(50% + 44px); width:calc(100% - 88px); height:2px; background: repeating-linear-gradient(90deg,#10B981 0,#10B981 6px,transparent 6px,transparent 14px); }
      `}</style>

      {/* ── NAVBAR ── */}
      <nav className="sticky top-0 z-50 bg-white/96 backdrop-blur-md border-b border-[#D1FAE5]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="#" className="flex items-center gap-2.5 font-bold text-[#065F46] text-lg">
            <div className="w-9 h-9 rounded-xl bg-[#059669] flex items-center justify-center text-white font-black text-sm select-none">PZ</div>
            ProprietárioZen
          </a>
          ...
          <button className="md:hidden p-2 rounded-lg hover:bg-[#F0FDF4]" onClick={() => setMobileOpen(v => !v)} aria-label="Menu">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="2" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
          </button>
        </div>
        {mobileOpen && (
      ...
      {/* ── HERO ── */}
      <section className="relative overflow-hidden" style={{background:'linear-gradient(135deg,#022C22 0%,#033D2C 55%,#044D38 100%)',padding:'88px 0 108px'}}>
        <div className="absolute -top-40 -right-20 w-[520px] h-[520px] rounded-full opacity-[0.055]" style={{background:'#34D399'}} />
        <div className="absolute -bottom-24 -left-12 w-[300px] h-[300px] rounded-full opacity-[0.055]" style={{background:'#34D399'}} />
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full text-[#34D399] text-xs font-semibold" style={{background:'rgba(52,211,153,0.12)',border:'1px solid rgba(52,211,153,0.25)'}}>
                <span className="w-1.5 h-1.5 rounded-full bg-[#34D399] lp-blink" />
                Recibos PDF automáticos — novidade
              </div>
              <h1 className="text-[52px] lg:text-[60px] font-bold text-white leading-[1.1] mb-6">
                Gerencie seus<br />imóveis <span className="text-[#34D399]">sem planilha,<br />sem estresse.</span>
              </h1>
              <p className="text-[17px] text-white/60 mb-10 leading-[1.75] max-w-[420px]">
                Controle aluguéis, inquilinos e cobranças num só lugar. Alertas automáticos de vencimento e recibos profissionais com 1 clique.
              </p>
              <div className="flex flex-wrap gap-4 mb-12">
                <Link href="/cadastro" className="inline-flex items-center gap-2.5 bg-[#10B981] hover:bg-[#059669] text-white font-semibold px-7 py-3.5 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(16,185,129,0.35)] text-[15px]">
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
            ...
            <div className="relative flex justify-center">
              <div className="lp-float relative">
                <div className="lp-badge-1 absolute top-[10%] -left-[28%] bg-white rounded-2xl shadow-2xl px-4 py-3 flex items-center gap-3 z-10 hidden lg:flex">
                ...
                <div className="lp-badge-2 absolute bottom-[30%] -right-[22%] bg-white rounded-2xl shadow-2xl px-4 py-3 flex items-center gap-3 z-10 hidden lg:flex">
                ...
                <div className="lp-badge-3 absolute top-[55%] -left-[32%] bg-white rounded-2xl shadow-2xl px-4 py-3 flex items-center gap-3 z-10 hidden lg:flex">
                  <div className="w-8 h-8 rounded-lg bg-[#D1FAE5] flex items-center justify-center shrink-0">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.5" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                  </div>
      ...
      {/* ── SOCIAL PROOF ── */}
      <section className="py-10 bg-[#F0FDF4] border-y border-[#D1FAE5]">
        <div className="max-w-6xl mx-auto px-6">
      ...
      <section className="py-24" id="funcionalidades">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-16 lp-fade">
            ...
            <h2 className="text-[40px] font-bold text-[#1F2937] leading-[1.2] mt-3 mb-4">Tudo para gerir seus imóveis<br />sem complicação.</h2>
            <p className="text-[17px] text-[#6B7280] max-w-lg leading-relaxed">Sem papel. Sem planilha. Sem surpresa.</p>
          </div>
      ...
      {/* ── HOW IT WORKS ── */}
      <section className="py-24 bg-[#F0FDF4]" id="como-funciona">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16 lp-fade">
            ...
            <h2 className="text-[40px] font-bold text-[#1F2937] leading-[1.2] mt-3">Comece a usar em minutos</h2>
            <p className="text-[17px] text-[#6B7280] mt-3 max-w-md mx-auto">Sem instalação, sem configuração complicada.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { n:'01', icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>, title:'Crie sua conta', desc:'Cadastro gratuito em menos de 1 minuto. Só precisa do seu e-mail.' },
              { n:'02', icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>, title:'Cadastre seus imóveis', desc:'Adicione imóveis e vincule os inquilinos. O app organiza tudo automaticamente.' },
              { n:'03', icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>, title:'Receba no prazo', desc:'Acompanhe pagamentos, receba alertas e gere recibos com 1 clique.' },
            ].map(({ n, icon, title, desc }, i) => (
              <div key={n} className={`lp-fade lp-delay-${i} text-center relative`}>
                {i < 2 && (
                  <div className="hidden md:block absolute top-10 left-[calc(50%+44px)] w-[calc(100%-44px)] overflow-hidden" style={{height:'2px',background:'repeating-linear-gradient(90deg,#10B981 0,#10B981 6px,transparent 6px,transparent 14px)'}} />
                )}
                <div className="relative flex justify-center mb-6">
                  ...
                  <span className="absolute -top-2 left-[calc(50%+28px)] w-6 h-6 rounded-full bg-[#059669] text-white text-[11px] font-bold flex items-center justify-center">{n}</span>
                </div>
      ...
      <section className="py-24" id="depoimentos">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16 lp-fade">
            ...
            <h2 className="text-[40px] font-bold text-[#1F2937] leading-[1.2] mt-3">O que dizem nossos proprietários</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={name} className={`lp-fade lp-delay-${delay} lp-card bg-white rounded-2xl border border-[#E5F7F0] p-8`}>
                <div className="flex gap-0.5 mb-5">
                  {Array.from({length:5}).map((_,i) => <span key={i} className="text-[#F59E0B] text-base">★</span>)}
                </div>
                <p className="text-[14px] text-[#374151] leading-[1.85] mb-6 italic">{text}</p>
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold shrink-0" style={{background:bg,color:fg}}>{initials}</div>
      ...
      {/* ── COMPARISON ── */}
      <section className="py-24" style={{background:'linear-gradient(135deg,#022C22 0%,#033D2C 100%)'}}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16 lp-fade">
            <span className="text-xs font-semibold text-[#34D399] uppercase tracking-widest">Diferenciais</span>
            <h2 className="text-[40px] font-bold text-white leading-[1.2] mt-3">Chega de improvisar</h2>
            <p className="text-[17px] mt-3 max-w-md mx-auto" style={{color:'rgba(255,255,255,0.55)'}}>Compare o jeito antigo com o ProprietárioZen.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="lp-fade rounded-2xl p-10" style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)'}}>
              <h3 className="text-lg font-bold mb-6 flex items-center gap-2" style={{color:'rgba(255,255,255,0.65)'}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                Gestão manual
              </h3>
              {['Planilhas manuais que viram bagunça','Esquece de cobrar, perde dinheiro','Recibos feitos no Word sem padrão','Reajuste depende de você lembrar e calcular','Dados espalhados em e-mail e caderno'].map(item => (
                <div key={item} className="flex items-start gap-3 mb-4">
                  <svg className="shrink-0 mt-0.5" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
            ...
            <div className="lp-fade lp-delay-1 rounded-2xl p-10 relative overflow-hidden" style={{background:'rgba(16,185,129,0.1)',border:'1px solid rgba(16,185,129,0.35)'}}>
              ...
              {['Dashboard organizado com visão de tudo','Alertas automáticos por e-mail no vencimento','Recibos PDF profissionais em 1 clique','Reajuste automático por IGPM ou IPCA','Tudo centralizado, acessível de qualquer lugar'].map(item => (
                <div key={item} className="flex items-start gap-3 mb-4">
                  <svg className="shrink-0 mt-0.5" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#34D399" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
      ...
      <section className="py-24" id="precos">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16 lp-fade">
            ...
            <h2 className="text-[40px] font-bold text-[#1F2937] leading-[1.2] mt-3">Simples e transparente</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
            ...
            <div className="lp-fade lp-card bg-white rounded-2xl border-2 border-[#E5F7F0] p-10">
              ...
              <p className="text-sm text-[#6B7280] mb-8">Para começar a organizar</p>
              <ul className="space-y-3 mb-8">
                {[['1 imóvel cadastrado',true],['Controle de inquilinos',true],['Histórico de aluguéis',true],['Recibos PDF ilimitados',false],['Alertas automáticos por e-mail',false],['Reajuste automático',false]].map(([f,ok]) => (
                  <li key={f as string} className={`flex items-center gap-2.5 text-sm ${ok ? 'text-[#374151]' : 'text-[#9CA3AF]'}`}>
                    {ok
                      ? <div className="w-4 h-4 rounded-full bg-[#D1FAE5] flex items-center justify-center shrink-0"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg></div>
                      : <div className="w-4 h-4 rounded-full bg-[#F1F5F9] flex items-center justify-center shrink-0"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div>
                    }
                    {f}
                  </li>
                ))}
              </ul>
              <Link href="/cadastro" className="block text-center border-2 border-[#10B981] text-[#059669] font-semibold py-3 rounded-xl hover:bg-[#F0FDF4] transition-colors text-sm">Começar grátis</Link>
            </div>
            {/* Pro */}
            <div className="lp-fade lp-delay-1 lp-card bg-white rounded-2xl border-2 border-[#10B981] p-10 relative shadow-[0_8px_32px_rgba(16,185,129,0.18)]">
              ...
              <p className="text-sm text-[#6B7280] mb-8">Gestão completa de imóveis</p>
              <ul className="space-y-3 mb-8">
      ...
      {/* ── CTA FINAL ── */}
      <section className="relative overflow-hidden py-28 text-center" style={{background:'linear-gradient(135deg,#065F46 0%,#022C22 100%)'}}>
        <div className="lp-blob-1 absolute w-80 h-80 rounded-full -top-20 -left-20 pointer-events-none" style={{background:'rgba(52,211,153,0.1)',filter:'blur(72px)'}} />
        <div className="lp-blob-2 absolute w-64 h-64 rounded-full -bottom-16 -right-16 pointer-events-none" style={{background:'rgba(16,185,129,0.09)',filter:'blur(64px)'}} />
        <div className="max-w-6xl mx-auto px-6 relative z-10 lp-fade">
          <h2 className="text-[48px] lg:text-[56px] font-bold text-white leading-[1.12] mb-4">
            Pare de improvisar.<br /><span className="text-[#34D399]">Comece a controlar.</span>
          </h2>
          <p className="text-lg mb-10" style={{color:'rgba(255,255,255,0.55)'}}>Cadastro gratuito em 1 minuto. Sem cartão de crédito.</p>
          ...
          <div className="grid md:grid-cols-[2fr_1fr_1fr_1fr] gap-12 mb-12">
            <div>
              <div className="flex items-center gap-2.5 font-bold text-white text-lg mb-3">
                <div className="w-9 h-9 rounded-xl bg-[#059669] flex items-center justify-center text-white font-black text-sm">PZ</div>
                ProprietárioZen
              </div>
              <p className="text-sm leading-relaxed max-w-[260px]" style={{color:'rgba(255,255,255,0.4)'}}>Gestão de imóveis simples e eficiente para proprietários brasileiros.</p>
            </div>
            {[
          ...
          <div className="border-t pt-6 flex flex-col sm:flex-row items-center justify-between gap-4" style={{borderColor:'rgba(255,255,255,0.08)'}}>
            ...
            <div className="flex gap-3">
              {[
                ...
                <a key={label as string} href="#" aria-label={label as string} className="w-9 h-9 rounded-lg flex items-center justify-center transition-all hover:bg-[#059669]" style={{background:'rgba(255,255,255,0.08)',color:'rgba(255,255,255,0.45)'}}>
                  {icon}
                </a>
              ))}
            </div>
